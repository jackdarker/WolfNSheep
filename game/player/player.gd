...
## angle in degree
@export var fixed_camera_pos:=Vector3(20,0,0.0)
...
@onready var _camera: Camera3D = %Camera3D				#Note adjust spring length to change distance of cam
...
func _ready() -> void:
	...
	Global.flag_reached.connect(func on_flag_reached() -> void:
...
# make sure mouse input wasnt handled already !
